...
class style extends admin {
	...
	public function __construct() {
		$this->filepath = JAE_PATH.'jae'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR;
		parent::__construct();
	}
	...
	public function export() {
		...
		if (file_exists($filepath)) {
			$arr = include $filepath;
			if (jae_base::load_config('system', 'charset') == 'gbk') {
				$arr = array_iconv($arr);
			}
			$data = base64_encode(json_encode($arr));
		    header("Content-type: application/octet-stream");
		    header("Content-Disposition: attachment; filename=jae_template_".$style.'.txt');
		    echo $data;
		} else {
