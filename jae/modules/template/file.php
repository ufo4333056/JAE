...
jae_base::load_sys_class('format', '', 0);
class file extends admin {
	...
	public function __construct() {
		...
		$this->filepath = JAE_PATH.'jae'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$this->style.DIRECTORY_SEPARATOR;
		...
		$this->tpl_edit = jae_base::load_config('system', 'tpl_edit');
		parent::__construct();
	}
	...
	if(empty($dir))	{$list= $this->style_info['file_explan']['templates|'.$this->style.'|'];}else {
		$list= $this->style_info['file_explan']['templates|'.$this->style.'|'.$dir];
		}
		
		
		if(!empty($list)) ksort($list);
		$local = str_replace(array(JAE_PATH, DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR), array('',DIRECTORY_SEPARATOR), $filepath);
		if (substr($local, -1, 1) == '.') {
	...
	public function public_ajax_get() {
		$op_tag = jae_base::load_app_class($_GET['op']."_tag", $_GET['op']);
		$html = $op_tag->$_GET['action']($_GET['html'], $_GET['value'], $_GET['id']);
		echo $html;
	}
	
	public function edit_jae_tag() {
		if (empty($this->tpl_edit)) {
			showmessage(L('tpl_edit'));
		}
		...
		switch ($op) {
			...
			case 'json':
				if ($_POST['dosubmit']) {
					...
					$jae_tag = creat_jae_tag($op, array('url'=>$url, 'cache'=>$cache, 'return'=>$return));
					if (in_array($tag_md5, $tag_md5_list[0])) {
						$old_jae_tag = $tag_md5_list[1][$tag_md5];
					}
					if (replace_jae_tag($filepath, $old_jae_tag, $jae_tag, $this->style, $dir)) {
				...
				include $this->admin_tpl('jae_tag_tools_json_xml');
				break;
			...
			case 'get':
				if ($_POST['dosubmit']) {
					...
					$return = isset($_POST['return']) && trim($_POST['return']) ? trim($_POST['return']) : '';
					$tag_md5_list = tag_md5($filepath);
					$jae_tag = creat_jae_tag($op, array('sql'=>$sql, 'dbsource'=>$dbsource, 'cache'=>$cache, 'return'=>$return));
					if (in_array($tag_md5, $tag_md5_list[0])) {
						$old_jae_tag = $tag_md5_list[1][$tag_md5];
					}
					if (replace_jae_tag($filepath, $old_jae_tag, $jae_tag, $this->style, $dir)) {
				...
				$dbsource_db = jae_base::load_model('dbsource_model');
				$r = $dbsource_db->select('', 'name');
				$dbsource_list = array(''=>L('please_select'));
				foreach ($r as $v) {
					$dbsource_list[$v['name']] = $v['name'];
				}
				include $this->admin_tpl('jae_tag_tools_get');
				break;
				
			default:
				if (!file_exists(JAE_PATH.DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.$op.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.$op.'_tag.class.php')) {
					showmessage(L('the_module_will_not_support_the_operation'));
				}
				$op_tag = jae_base::load_app_class($op."_tag", $op);
				if (!method_exists($op_tag, 'jae_tag')) {
					showmessage(L('the_module_will_not_support_the_operation'));
				}
				$html  = $op_tag->jae_tag();
				if ($_POST['dosubmit']) {
				...
				include $this->admin_tpl('jae_tag_modules');
				break;
		}
	}
}