...
class template_bak extends admin {
	...
	public function __construct() {
		...
		$this->filepath = JAE_PATH.'jae'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$this->style.DIRECTORY_SEPARATOR.$this->dir.DIRECTORY_SEPARATOR.$this->filename;
		$this->fileid = $this->style.'_'.$this->dir.'_'.$this->filename;
		$this->tpl_edit = jae_base::load_config('system', 'tpl_edit');
		$this->db = jae_base::load_model('template_bak_model');
	...
	public function restore() {
		...
		if ($data = $this->db->get_one(array('id'=>$id))) {
			...
			if (@file_put_contents($this->filepath, htmlspecialchars_decode($data['template']))) {
