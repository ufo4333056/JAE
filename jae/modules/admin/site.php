...
class site extends admin {
	...
	public function __construct() {
		$this->db = jae_base::load_model('site_model');
		parent::__construct();
		$this->menuid=77;
	}
	...
	public function add() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			$appkey = isset($_POST['appkey']) && trim($_POST['appkey']) ? trim($_POST['appkey']) : '';
			$secretkey = isset($_POST['secretkey']) && trim($_POST['secretkey']) ? trim($_POST['secretkey']) : '';
			...
			$default_style = isset($_POST['template']) && !empty($_POST['template']) ? $_POST['template'] : showmessage(L('please_choose_the_default_style'));			   
			...
			if ($this->db->insert(array('name'=>$name,'dirname'=>$dirname, 'domain'=>$domain, 'site_title'=>$site_title, 'keywords'=>$keywords, 'description'=>$description,'appkey'=>$appkey,'secretkey'=>$secretkey, 'release_point'=>$release_point, 'template'=>$template,'setting'=>$setting, 'default_style'=>$default_style))) {
				$class_site = jae_base::load_app_class('sites');
			...
			$release_point_db = jae_base::load_model('release_point_model');
	...
	public function edit() {  
		...
		if ($data = $this->db->get_one(array('siteid'=>$siteid))) {
			if (isset($_POST['dosubmit'])) {
				...
				$release_point = isset($_POST['release_point']) ? $_POST['release_point'] : '';
				$template = isset($_POST['template']) && !empty($_POST['template']) ? $_POST['template'] : showmessage(L('please_select_a_style'));
				$default_style =isset($_POST['template']) && !empty($_POST['template']) ? $_POST['template'] : showmessage(L('please_select_a_style'));	
				...
				$sql = array('name'=>$name,'dirname'=>$dirname, 'domain'=>$domain, 'site_title'=>$site_title, 'keywords'=>$keywords, 'description'=>$description, 'appkey'=>$appkey, 'secretkey'=>$secretkey, 'release_point'=>$release_point, 'template'=>$template, 'setting'=>$setting, 'default_style'=>$template);
				...
				if ($this->db->update($sql, array('siteid'=>$siteid))) {
					...
					showmessage(L('operation_success'), '/admin.php?m=admin&c=site&a=init&menuid=77', '1.2', 'edit');
				} else {
					showmessage(L('operation_failure'));
				}
			} else {
	...
	public function public_name() {
		$name = isset($_GET['name']) && trim($_GET['name']) ? (jae_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['name'])) : trim($_GET['name'])) : exit('0');
	...
	public function public_dirname() {
		$dirname = isset($_GET['dirname']) && trim($_GET['dirname']) ? (jae_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['dirname'])) : trim($_GET['dirname'])) : exit('0');
		$siteid = isset($_GET['siteid']) && intval($_GET['siteid']) ? intval($_GET['siteid']) : '';
		$data = array();
		if ($siteid) {
	...
	public function set_site() {
		$siteid = isset($_GET['siteid']) && intval($_GET['siteid']) ? intval($_GET['siteid']) : showmessage(L('illegal_parameters'), HTTP_REFERER);
		$this->db->update(array('uuid'=>'0'),'siteid != 0 ');
		$this->db->update(array('uuid'=>'1'),'siteid='.$siteid );
		$class_site = jae_base::load_app_class('sites');
		$class_site->set_cache();					
		showmessage(L('operation_success'), '/admin.php?m=admin&c=site&a=init&menuid=77', '1.2', 'edit');
