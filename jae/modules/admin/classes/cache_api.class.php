...
class cache_api {
	...
	public function cache($model = '', $param = '') {
		...
		if (file_exists(JAE_PATH.'jae'.DIRECTORY_SEPARATOR.'model'.DIRECTORY_SEPARATOR.$model.'_model.class.php')) {

			$this->db = jae_base::load_model($model.'_model');
			if ($param) {
				$this->$model($param);
			} else {
				$this->$model();
	...
	public function cache_siteid($role) {
		$priv_db = jae_base::load_model('admin_role_priv_model');
		$sitelist = array();
		foreach($role as $n=>$r) {
	...
	public function sitemodel_field($modelid) {
		$field_array = array();
		$db = jae_base::load_model('sitemodel_field_model');
	...
	public function dbsource() {
		$db = jae_base::load_model('dbsource_model');
		$list = $db->select();
		$data = array();
		if ($list) {
			foreach ($list as $val) {
	...
	public function member_setting() {
		$this->db = jae_base::load_model('module_model');
	...
	public function membermodel() {
		...
		$sitemodel_db = jae_base::load_model('sitemodel_model');
	...
	public function member_model_field() {
		...
		$this->db = jae_base::load_model('sitemodel_field_model');
	...
	public function special() {
		$specials = array();
		$result = $this->db->select(array('status'=>0), '*', '', '`listorder` DESC, `id` DESC');
		foreach($result as $r) {
			$specials[$r['id']] = $r;
		}
	...
	public function database() {
		...
		return $file_size = jae_base::load_config('system','lock_ex') ? file_put_contents($filepath.'modules.php', $module, LOCK_EX) : file_put_contents($filepath.'modules.php', $module);
	}
	...
	public function cache2database() {
		...
		$cache = jae_base::load_model('cache_model');
		...
		if (is_array($result) && !empty($result)) {
			foreach ($result as $re) {
				...
				if (!file_exists(CACHE_PATH.$re['path'].$re['filename'])) {
					$filesize = jae_base::load_config('system','lock_ex') ? file_put_contents(CACHE_PATH.$re['path'].$re['filename'], $re['data'], LOCK_EX) : file_put_contents(CACHE_PATH.$re['path'].$re['filename'], $re['data']);
				} else { file_put_contents(CACHE_PATH.$re['path'].$re['filename'], $re['data']);
					continue;
				}
			}
		}
		...
		if ($currpage>$pages) {
			...
			showmessage(L('part_cache_success'), '/admin.php?m=admin&c=cache_all&a=init&page='.$_GET['page'].'&currpage='.$currpage.'&pages='.$pages.'&dosubmit=1',0);
		}
	}
	...
	public function video_category_tb() {
		if (module_exists('video')) {
			...
			jae_base::load_app_class('ku6api', 'video', 0);
