...
class cache_all extends admin {
	...
	public function init() {
		if (isset($_POST['dosubmit']) || isset($_GET['dosubmit'])) {
			...
			$this->cache_api = jae_base::load_app_class('cache_api', 'admin');
			$m = $modules[$page];
			if ($m['mod'] && $m['function']) {
				if ($m['file'] == '') $m['file'] = $m['function'];
				$M = getcache('modules', 'commons');
				if (in_array($m['mod'], array_keys($M))) {
					$cache = jae_base::load_app_class($m['file'], $m['mod']);
					$cache->$m['function']();
				}
			...
			if (!empty($modules[$page])) {
				...
				showmessage(L('update').$m['name'].L('cache_file_success'), '/admin.php?m=admin&c=cache_all&page='.$page.'&dosubmit=1', 0);
			} else {
				//echo '<script type="text/javascript">window.parent.addtext("<li>'.L('update').$m['name'].L('site_cache_success').'..........</li>")</script>';
				showmessage(L('update').$m['name'].L('site_cache_success'), '/admin.php');
			}
		} else {
