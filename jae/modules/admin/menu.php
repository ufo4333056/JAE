...
class menu extends admin {
	...
	function init () {
		...
		foreach($result as $r) {
			$r['cname'] = L($r['name']);
			$r['str_manage'] = '<a href="/admin.php?aid='.$_GET['aid'].'&cid='.$_GET['cid'].'&did='.$_GET['did'].'&m=admin&c=menu&a=add&parentid='.$r['id'].'&menuid='.$_GET['menuid'].'">'.L('add_submenu').'</a> | <a href="/admin.php?aid='.$_GET['aid'].'&cid='.$_GET['cid'].'&did='.$_GET['did'].'&m=admin&c=menu&a=edit&id='.$r['id'].'&menuid='.$_GET['menuid'].'">'.L('modify').'</a> | <a href="/admin.php?m=admin&c=menu&a=delete&id='.$r['id'].'&menuid='.$_GET['menuid'].'">'.L('delete').'</a> ';
			$array[] = $r;
		}
	...
	function edit() {
		...
		//	$r = $this->db->get_one('*','jae_mennu','id='.$id);
			$data=$_POST['info'];
			...
			$tree = jae_base::load_sys_class('tree');
			$id = intval($_GET['id']);
			//$r = $this->db->get_one('*',$this->table,array('id'=>$id));
