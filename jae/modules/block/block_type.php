...
class block_type extends admin {
	function __construct() {
		parent::__construct();
		$this->db =jae_base::load_sys_class('mysql');
		$this->table='jae_block_type';
		$this->menuid=62;
	}
	...
	function init () {
		...
		$pages=$this->db->pages;			
		include $this->admin_tpl('block_type_list','block');
	}
	function add() {
		$sitelist=getcache('sitelist','commons');
		if(isset($_POST['dosubmit'])) {
			...
			include $this->admin_tpl('block_type_add');
			
		} else {		
			
			include $this->admin_tpl('block_type_add','block');
		}
	}
	function delete() {
	...
	function edit() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($r) extract($r);			
			include $this->admin_tpl('block_type_edit');
