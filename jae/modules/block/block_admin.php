...
class block_admin extends admin {
	...
	public function __construct() {
		...
		$this->priv_db = jae_base::load_model('block_priv_model');
		$this->history_db = jae_base::load_model('block_history_model');
	...
	public function del() {
		...
		if ($this->db->delete(array('id'=>$id)) && $this->history_db->delete(array('blockid'=>$id)) && $this->priv_db->delete(array('blockid'=>$id))) {
			if (jae_base::load_config('system','attachment_stat')) {
				$this->attachment_db = jae_base::load_model('attachment_model');
				$keyid = 'block-'.$id;
	...
	public function block_update() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($data['type'] == 2) {
				...
				if ($template) {
					$block = jae_base::load_app_class('block_tag');
	...
	public function public_visualization() {
		...
		jae_base::load_app_func('global','template');
		ob_start();
		...
		echo "<link rel=\"stylesheet\" type=\"text/css\" href=\"/statics/css/admin_visualization.css\" />";
	...
	public function public_search_content() {
		...
		if (isset($_GET['dosubmit']) && !empty($catid)) {
			...
			$content_db = jae_base::load_model('content_model');
