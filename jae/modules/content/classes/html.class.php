<?php
defined('IN_JAE') or exit('No permission resources.');defined('IN_JAE') or exit('No permission resources.');
//jae_base::load_app_func('util','content');
//jae_base::load_sys_func('dir');
class html {
	private $siteid,$url,$html_root,$queue,$categorys;
	public function __construct() {
		//$this->queue = jae_base::load_model('queue_model');
		//define('HTML',true);
		//self::set_siteid();
		//$this->categorys = getcache('category_content_'.$this->siteid,'commons');
		//$this->url = jae_base::load_app_class('url', 'content');
		//$this->html_root = jae_base::load_config('system','html_root');
 ...
 */
	...
	*/
	...
	public function category($catid, $page = 0) {
		...
		if($create_to_html_root) {
			if($this->siteid==1) {
				$file = JAE_PATH.$base_file;
			} else {
				$file = JAE_PATH.substr($this->html_root,1).$base_file;
			}
			//添加到发布点队列
	...
	*/
	private function createhtml($file, $copyjs = '') {
		...
		if ($copyjs && !file_exists($dir.'/js.html')) {
			@copy(jae_PATH.'modules/content/templates/js.html', $dir.'/js.html');
		}*/
		//$strlen = file_put_contents($file, $data);
		
		$saveTextFileResult = $fileStoreService->saveTextFile($data,"index.html") ;
		
		//@chmod($file,0777);
		if(!is_writable($file)) {
			$file = str_replace(JAE_PATH,'',$file);
