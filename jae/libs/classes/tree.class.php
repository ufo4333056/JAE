...
class tree {
	...
	public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = ''){
		...
		if(is_array($child)){
			...
			foreach($child as $id=>$value){
				...
				@extract($value);
				$str  = "<tr>
					<td align='center'><input name='listorders[".$id."]' type='text' size='3' value='".$listorder."' class='input-text-c input-text'></td>
	...
	* @return string
	*/
	public function get_option($myid, $str, $sid = 0, $adds = '', $str_group = ''){
		$number=1;
		$child = $this->get_child($myid);
		if(is_array($child)){
		      $total = count($child);
			foreach($child as $id=>$value){
				...
				$this->ret .= $nstr;
				$nbsp = $this->nbsp;
				$this->get_option($id, $str, $sid, $adds.$k.$nbsp,$str_group);
				$number++;
			}
		}
		return $this->ret;
