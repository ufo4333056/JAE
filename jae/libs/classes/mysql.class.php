...
final class mysql {
	...
	public function __construct() {
		//$this->db= new PDO('mysql:host=localhost;port=3306;dbname=hubei', 'root', '123456');
		$this->db= new PDO();
		$this->lastqueryid = 1;
	...
	public function open($config) { 
		$sql="SELECT * FROM focus WHERE  status=1 ORDER BY listorder ASC limit 1  ";
	...
	private function execute($sql) {
		...
		//echo $sql;	
		$this->lastqueryid = $this->db->prepare($sql);
		$this->lastqueryid->execute();	
		$sql_info =$this->lastqueryid->errorinfo(); //取得错误信息数组(注意此处取的是$pdo的errorInfo而不是$sql的)
		if($sql_info[1] != 00000) { echo '执行：' . $sql_info[2] ;$appLog->info($sql_info[2]);}; //输出错误信息
		
	
 		//$this->lastqueryid = $this->lastqueryid->fetchAll(PDO::FETCH_ASSOC);
		$this->querycount++;
	 ...
	 * @return unknown_type
	 */
	public function listinfo($data="*",$table,$where = '', $order = '', $page = 1, $pagesize = 20, $key='', $setpages = 10,$urlrule = '',$array = array()) {
		$where = to_sqls($where);
		//$this->number = $this->count($where);
		//$r = $this->get_one('COUNT(*) AS num',$table,$where);
		if(empty($where)) $where=1;
		
		$num=$this->db->query("select count(*) from ".$table." WHERE ". $where)->fetchColumn();
		...
		if ($this->number > 0) {
			...
			return $this->select($data="*",$table,$where, "$offset, $pagesize", $order, '', $key);
		} else {
			return array();
		}
	}
	...
	public function get_one($data, $table, $where = '', $order = '', $group = '') {
		...
		//$res = $this->fetch_next();
		$this->free_result();
		foreach($this->lastqueryid as $key=>$val )
		{$datalist=$val;
			}
	...
	public function insert($data, $table, $return_insert_id = false, $replace = false) {
		...
		//$sql="INSERT INTO `jae_menu`(`name`,`listorder`) VALUES ('中国',11)";
		$return = $this->execute($sql);
