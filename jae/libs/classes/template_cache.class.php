...
final class template_cache {
	...
	public function template_compile($module, $template, $style = 'default') {
		...
		$tplfile = $_tpl = JAE_PATH.'jae'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html';
			...
			$module = str_replace('/', DIRECTORY_SEPARATOR, $module);
			$tplfile = $_tpl = JAE_PATH.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$template.'.html';
		}
		if ($style != 'default' && !file_exists ( $tplfile )) {
			$style = 'default';
			$tplfile = JAE_PATH.'jae'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html';
		}
	...
	public function template_parse($str) {
		...
		$str = preg_replace("/\{pc:(\w+)\s+([^}]+)\}/e","jae_tag('$1','$2', '$0')", $str);
		$str = preg_replace("/\{\/pc\}/", self::end_jae_tag() , $str);
		$str = "<?php defined('IN_JAE') or exit('No permission resources.'); ?>" . $str;
		return $str;
	}
	 ...
	 */
	public static function jae_tag($op, $data, $html) {
		...
		if (in_array($op,$tools)) {
			...
			switch ($op) {
				...
				case 'xml':
						$str .= '$xml = jae_base::load_sys_class(\'xml\');';
						$str .= '$xml_data = @file_get_contents(\''.$datas['url'].'\');';
						$str .= '$'.$return.' = $xml->xml_unserialize($xml_data);';
					break;
					
				case 'get':
						$str .= 'jae_base::load_sys_class("get_model", "model", 0);';
						if ($datas['dbsource']) {
				...
				case 'block':
					...
					$str .= '$block_tag = jae_base::load_app_class(\'block_tag\', \'block\');';
					$str .= 'echo $block_tag->jae_tag('.self::arr_to_html($datas).');';
					break;
			}
		} else {
			if (!isset($action) || empty($action)) return false;			
			
			if (module_exists($op) && file_exists(JAE_PATH.DIRECTORY_SEPARATOR.'jae'.DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.$op.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.$op.'_tag.class.php')) {
				
				$str .= '$'.$op.'_tag = jae_base::load_app_class("'.$op.'_tag", "'.$op.'");if (method_exists($'.$op.'_tag, \''.$action.'\')) {';	
				if (isset($start) && intval($start)) {
		...
		return "<"."?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo \"<div class=\\\"admin_piao\\\" data-action=\\\"".$op."\\\" data-datas=\\\"".$str_datas."\\\"><a href=\\\"/admin.php?m=visualization&c=".$op."&a=".$action."&".$str_datas."\\\" target=\\\"_blank\\\" class=\\\"admin_piao_edit\\\">".($op=='block' ? L('EDIT') : L('EDIT'))."</a>\";}".$str."?".">";
	}
	
	/**
	 * PC标签结束
	 */
	static  function end_jae_tag() {
		return '<?php if(defined(\'IN_ADMIN\') && !defined(\'HTML\')) {echo \'<div style=clear:both></div></div>\';}?>';
	}
